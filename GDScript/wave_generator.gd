...
func init_gpu(num_cascades : int) -> void:
	...
	pipelines[&'spectrum_compute'] = context.create_pipeline([float(map_size)/16, float(map_size)/16, 1], [spectrum_set], spectrum_compute_shader)
	pipelines[&'spectrum_modulate'] = context.create_pipeline([float(map_size)/16, float(map_size)/16, 1], [spectrum_set, fft_buffer_set], spectrum_modulate_shader)
	pipelines[&'fft_butterfly'] = context.create_pipeline([float(map_size)/2/64, num_fft_stages, 1], [fft_butterfly_set], fft_butterfly_shader)
	pipelines[&'fft_compute'] = context.create_pipeline([1, float(map_size), 4], [fft_compute_set], fft_compute_shader)
	pipelines[&'transpose'] = context.create_pipeline([float(map_size)/32, float(map_size)/32, 4], [fft_compute_set], transpose_shader)
	pipelines[&'fft_unpack'] = context.create_pipeline([float(map_size)/16, float(map_size)/16, 1], [unpack_set, fft_buffer_set], fft_unpack_shader)
